...
const { getTopEvents } = require('./events');
...
const EXHAUST_MAP = {
  '気を遣いすぎる': '相手の感情を先読みして動く反面、自分の感情の置き場を見失いやすい傾向があります。',
  '本音が言えない': '言葉を選びすぎて、伝えたいことの核心が届かないことがある傾向があります。',
  '深くなるのが怖い': '距離が縮まるほど、逆に自分から引いてしまう動きが出やすい傾向があります。',
  '一人でいると不安': '誰かとつながっているときの自分と、一人のときの自分に大きな落差がある傾向があります。',
  '期待して裏切られる': '相手への期待値が高い分、ギャップで消耗するサイクルに入りやすい傾向があります。',
};
...
function detectRegionType(birthplace) {
  ...
  for (const k of URBAN_KEYWORDS) {
    if (birthplace.includes(k)) return 'urban';
  }
  return 'rural';
}
...
function makeQ1QuickReply(events) {
  const items = events.map(e => ({
    type: 'action',
    action: { type: 'message', label: e.label, text: e.label }
  }));
  items.push({ type: 'action', action: { type: 'message', label: 'ここにない', text: 'ここにない' } });
  return {
    type: 'text',
    text: '10代〜20代で、一番しんどかった出来事を選んでください。\n（あなたの時代と背景に合わせて絞り込みました）',
    quickReply: { items }
  };
}

function reaskStep(session) {
  switch (session.step) {
    case 0: return 'お名前を教えてください。';
    case 1: return (session.name||'')+'さん、もう一度生年月日を教えてください。\n例：2000/9/26 または 2000年9月26日';
    case 2: return { type:'text', text:'もう一度、性別を教えてください（任意）。', quickReply:{ items:[
    ...
    case 3: return { type:'text', text:'もう一度、出生時間帯を教えてください（任意）。', quickReply:{ items:[
    ...
    case 4: return 'もう一度、出生地（都道府県や市区町村）を教えてください。\n例：東京都、大阪市、石川県など';
    case 5: return session.topEvents ? makeQ1QuickReply(session.topEvents) : '「診断」と送るとはじめからやり直せます。';
    case 6: return { type:'text', text:'もう一度、人間関係で一番消耗するパターンを選んでください。', quickReply:{ items: Object.keys(EXHAUST_MAP).map(k=>({ type:'action', action:{ type:'message', label:k, text:k } })) }};
...
function getNextStep(session, text) {
  switch (session.step) {
    ...
    case 4: {
      ...
      session.step = 5;
      const topEvents = getTopEvents(session.birthdate.year, session.gender);
      session.topEvents = topEvents;
      return makeQ1QuickReply(topEvents);
    }
    case 5: {
      if (text === 'ここにない') {
        session.q1Label = null;
        session.q1Imprint = null;
      } else {
        const found = session.topEvents && session.topEvents.find(e => e.label === text);
        session.q1Label = found ? found.label : text;
        session.q1Imprint = found ? found.imprint : null;
      }
      session.step = 6;
      return { type:'text', text:'人間関係で一番消耗するパターンはどれですか？', quickReply:{ items: Object.keys(EXHAUST_MAP).map(k=>({ type:'action', action:{ type:'message', label:k, text:k } })) }};
    }
    case 6: {
      session.q2Key = text;
      session.q2Text = EXHAUST_MAP[text] || null;
      session.step = 7;
      const { year, month, day } = session.birthdate;
      const zn = ZODIAC_NAMES[getZodiac(month, day)];
      return 'ありがとうございます。確認します。\n\nお名前：'+session.name+'\n生年月日：'+year+'年'+month+'月'+day+'日'+'\n時間帯：'+session.timeOfDay+'\n出生地：'+session.birthplace+'\n星座：'+zn+'\n10代〜20代の体験：'+(session.q1Label||'選択なし')+'\n消耗パターン：'+text+'\n\nこの内容で診断を送ります。よろしいですか？（はい／いいえ）';
    }
    case 7:
      if (/はい|yes|ok/i.test(text)) { session.step = 8; return '__SEND__'; }
      session.step = 0;
      return 'はじめからやり直します。\n\nお名前を教えてください。';
    default:
...
async function handleMessage(event) {
  ...
  if (text === '戻って') {
    if (session.step <= 0) {
      await client.replyMessage(event.replyToken, { type:'text', text:'これ以上戻れません。\n最初の質問からはじめましょう。\n\nお名前を教えてください。' });
      session.step = 0;
      return;
    }
    session.step -= 1;
    const response = reaskStep(session);
