...
function getNextStep(session, text) {
    switch (session.step) {
      ...
      case 4: {
              // 出生地を受け取り、Q1（しんどかった時期）を表示
              session.birthplace = text.trim();
              session.step = 5;

              // 生年月日から上位4イベントを取得してセッションに保存
              const { year } = session.birthdate;
              const topEvents = getTopEvents(year, session.gender);
              session.topEvents = topEvents; // あとでimprint取得するために保存

              const quickItems = topEvents.map(e => ({
                        type: 'action',
                        action: { type: 'message', label: e.label, text: e.label }
              }));
              quickItems.push({
                        type: 'action',
                        action: { type: 'message', label: 'ここにない', text: 'ここにない' }
              });

              return {
                        type: 'text',
                        text: '10代〜20代で、一番しんどかったと感じる出来事はどれですか？',
      ...
      case 5: {
              ...
              if (text === 'ここにない') {
                        session.step = 5.5;
                        return 'どんな出来事でしたか？一言で教えてください。';
              }
              // 選択されたラベルからimprintを取得
              const matched = (session.topEvents || []).find(e => e.label === text);
              session.hardEventImprint = matched ? matched.imprint : '';
              session.step = 6;
              return buildQ2();
      }

      case 5.5: {
              // 自由記述の場合はimprintなし（テキストだけ保存）
              session.hardEventImprint = '';
              session.step = 6;
              return buildQ2();
      }
      ...
      case 6: {
              // Q2の回答を受け取る
              session.exhaustPattern = text.trim();
              session.step = 7;
...
function buildQ2() {
    return {
          type: 'text',
          text: '人間関係で一番消耗するのはどれですか？',
          quickReply: { items: [
            { type: 'action', action: { type: 'message', label: '気を遣いすぎる', text: '気を遣いすぎる' } },
            { type: 'action', action: { type: 'message', label: '本音が言えない', text: '本音が言えない' } },
            { type: 'action', action: { type: 'message', label: '深くなるのが怖い', text: '深くなるのが怖い' } },
            { type: 'action', action: { type: 'message', label: '一人でいると不安', text: '一人でいると不安' } },
            { type: 'action', action: { type: 'message', label: '期待して裏切られる', text: '期待して裏切られる' } },
                ]},
    };
}
...
async function handleMessage(event) {
  ...
  if (response === '__SEND__') {
        const { name, birthdate, birthplace, gender, hardEventImprint, exhaustPattern } = session;
        const { year, month, day } = birthdate;
        const zodiacKey = getZodiac(month, day);
        const regionType = detectRegionType(birthplace);

      // 消耗パターンのimprint取得
      const exhaustText = EXHAUST_MAP[exhaustPattern] || '';

      const resultText = buildZodiacText(name, zodiacKey, hardEventImprint, exhaustText);
        ...
        if (adminId) {
                ...
                await client.pushMessage(adminId, {
                          ...
                          text: '【新規診断】\\n名前：' + name +
                                      ...
                                      '\\n刻印：' + (hardEventImprint ? '取得済み' : 'なし/自由記述') +
                                      '\\n消耗パターン：' + (exhaustPattern || '未回答'),
                });
        }
